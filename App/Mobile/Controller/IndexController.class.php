...
class IndexController extends CommonController {
    ...
    public function index() {
        ...
        $print_table = M('print');
        $list = $picture_table->where(array('type' => 1, 'status' => 1))->order('id desc')->limit(3)->select();
        $printlist = $print_table->order('id')->order('id desc')->limit(4)->select();
        // var_dump($printlist);
        $this->assign('printlist', $printlist);
        $this->assign('list', $list);
        $map =$article_table->field(array('art_title','id','art_type','art_info','art_img'))->where( 'art_type=7')->order('id desc')->limit(4)->select();
        $this->assign('map', $map);
