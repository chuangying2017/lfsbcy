...
//        }
    ...
    public function indexadd(){
        $arr = I('post.','','htmlspecialchars,addslashes');
	
        $map1 = M('power')->field(array('pid','name'))->where('level=1')->group('pid')->select();
        $this->assign('map1',$map1);
        if(!empty($arr)) {
            $arr['level'] = '1';
            $bool = M('power')->data($arr)->add();
            if ($bool) {
                $this->success("添加成功");
            } else {
                $this->error("添加失败");
    ...
    public function indexdelete(){
        ...
        $map = M('power')->field(array('id','pid','name','son_id'))->where('level=1')->order('id desc')->select();;
        $this->assign('map',$map);
        if(!empty($arr)){
            $bool = M('power')->data($arr)->delete();
            if ($bool) {
                $this->success("添加成功");
            } else {
                $this->error("添加失败");
